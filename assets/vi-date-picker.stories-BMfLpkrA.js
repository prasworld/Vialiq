...
import { c as i, A, b, r, i as i$1 } from './iframe-dVqAHwik.js';
import { n, V as ViElement, t } from './vi-element-DltBFs1y.js';
import { r as r$1 } from './state-BDPry667.js';
...
import { V as ValidityMixin } from './validity-mixin-DvytLH3I.js';
import { F as FloatingController } from './keyboard-controller-DbV1C_E6.js';
import './vi-select-BS3LoVpM.js';
import { g as getDefaultExportFromCjs } from './_commonjsHelpers-B85MJLTf.js';
import './base-Cl6v8-BZ.js';
import './floating-ui.dom-DwUTpXgb.js';
import './focusable-mixin-CmxOyPX5.js';
import './if-non-empty-Dw5mszyO.js';
import './if-defined-D7D7f9f1.js';
import './vi-icon-DcY8xuJx.js';
...
function ViMonthYearPlugin(config = {}) {
    return function(fp) {
        ...
        function createHeader() {
            ...
            yearSelect.addEventListener('vi-select-change', (e)=>{
...
const datePickerInputStyles = "@charset \"UTF-8\";@layer reset,components,utilities;:host{display:block;font-family:var(--vi-font-family-base, var(--vi-font-family-base, -apple-system, BlinkMacSystemFont, \"Segoe UI\", Roboto, sans-serif))}.input-wrapper{display:flex;flex-direction:column;gap:var(--vi-spacing-xs, var(--vi-spacing-xs, .5rem))}.label{font-size:var(--vi-font-size-sm, var(--vi-font-size-sm, .8125rem));color:var(--vi-text-secondary, var(--vi-text-secondary, #4b5563));font-weight:var(--vi-font-weight-medium, var(--vi-font-weight-medium, 500))}:host([disabled]){cursor:not-allowed;opacity:var(--vi-disabled-opacity, .6)}:host([disabled]) .label,:host([disabled]) .trigger{pointer-events:none}.trigger{box-sizing:border-box;display:flex;align-items:center;justify-content:space-between;width:100%;min-height:var(--vi-input-sizing-min-height, 2rem);padding:var(--vi-input-spacing-padding-block, .25rem) var(--vi-input-spacing-padding-inline, .6875rem);background:var(--vi-input-bg, var(--vi-layer-01, #ffffff));border:1px solid var(--vi-input-trigger-border-color, var(--vi-border-02, #eeeeee));border-radius:var(--vi-border-radius-md, .375rem);color:var(--vi-text-primary, var(--vi-text-primary, #111827));font-family:inherit;font-size:var(--vi-input-typography-font-size, .875rem);text-align:left;cursor:pointer;transition:all .2s ease}.trigger:hover:not(:disabled){background:var(--vi-layer-hover-01, var(--vi-layer-hover-01, #f3f4f6));border-color:var(--vi-border-03, var(--vi-border-03, #e0e0e0))}:host(:focus-within) .trigger{outline:none;border-color:var(--vi-focus, var(--vi-focus, #3676d0));box-shadow:0 0 0 1px var(--vi-focus, var(--vi-focus, #3676d0))}:host([invalid]) .trigger{border-color:var(--vi-input-error-border-color, var(--vi-color-error, #ef4444))}:host([invalid]:focus-within) .trigger{border-color:var(--vi-input-error-border-color, var(--vi-color-error, #ef4444));box-shadow:0 0 0 1px var(--vi-input-error-border-color, var(--vi-color-error, #ef4444))}.trigger:disabled{background:var(--vi-layer-disabled, var(--vi-layer-02, #f3f4f6));color:var(--vi-text-disabled, var(--vi-text-disabled, #9e9e9e));cursor:not-allowed;border-color:var(--vi-border-disabled, var(--vi-border-02, #eeeeee))}.display-value{flex-grow:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.placeholder{color:var(--vi-text-placeholder, var(--vi-text-secondary, #4b5563))}.icon{flex-shrink:0;display:flex;align-items:center;color:var(--vi-text-secondary, var(--vi-text-secondary, #4b5563));margin-left:var(--vi-spacing-sm, var(--vi-spacing-sm, .75rem))}.trigger:disabled .icon{color:var(--vi-text-disabled, var(--vi-text-disabled, #9e9e9e))}.sr-only{position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap;border-width:0}";
...
/** Emitted when the user selects a date. Detail: DatePickerChangeDetail. */ const VI_DATE_PICKER_CHANGE = 'vi-date-picker-change';
let _ViDatePicker;
    ...
    static{
        class ViDatePicker extends (_ValidityMixin = ValidityMixin(FlatpickrMixin(ViElement))) {
            ...
            async updated(changed) {
                ...
                if (changed.has('disabled') || changed.has('status') || changed.has('validityMessage') || changed.has('required')) {
                    if (this._inputs) {
                        this._inputs.forEach((input)=>{
            ...
            _emitChange(dates, formattedValue) {
                ...
                this.dispatchEvent(new CustomEvent(VI_DATE_PICKER_CHANGE, {
                    detail,
                    bubbles: true,
                    composed: true
                }));
...
const render = ({ mode, flat, min, max, locale, disabled, required, weekNumbers, status, validityMessage, name, value, onViDatePickerChange })=>b`
  <div style="padding: 1.5rem; font-family: sans-serif;">
    <vi-date-picker
      ...
      min=${min || ''}
      max=${max || ''}
      @vi-date-picker-change=${(e)=>onViDatePickerChange?.(e.detail)}
    >
      ${mode === 'range' ? b`
...
const RangeMode = {
    ...
    parameters: {
        docs: {
            description: {
                story: 'Allows the user to select a start and end date. `vi-date-picker-change` detail includes both `rawValue` and `rawEndValue`.'
            }
        }
    },
    args: {
...
const MonthMode = {
    ...
    <div style="padding: 1.5rem; font-family: sans-serif;">
      <vi-date-picker
        ...
        min=${args.min || ''}
        max=${args.max || ''}
        @vi-date-picker-change=${(e)=>args.onViDatePickerChange?.(e.detail)}
      >
        <vi-date-picker-input
